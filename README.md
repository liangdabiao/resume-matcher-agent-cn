...
> 后端只用标准 `pip install -r requirements.txt`，不需要 uv / poetry 等包管理器，安装最简单。
...
> **自定义前端端口**：默认 3000。若要改用其他端口（如 3001），单独启动前端即可：`npm run dev:frontend -- -p 3001`，或在 `apps/frontend` 目录下 `npm run dev -- -p 3001`。后端 CORS 默认已放行 3000-3002，无需额外配置。服务器/宝塔部署改端口见 [`docs/BAOTA_DEPLOY.md`](./docs/BAOTA_DEPLOY.md) 第 5.0 节。
...
| 变量 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `ENV` | 否 | `local` | 可选：`local`、`production`；production 会强制校验下面的 Key/密钥 |
...
> 📖 **宝塔面板部署**：详见 [`docs/BAOTA_DEPLOY.md`](./docs/BAOTA_DEPLOY.md)，傻瓜式步骤，宝塔 Python 项目管理器原生支持。

后端是标准 Flask + Gunicorn，服务器/宝塔直接跑，无 ASGI/WSGI 坑：
